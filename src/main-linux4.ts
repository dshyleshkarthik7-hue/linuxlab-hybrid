...
async function boot(): Promise<void> {
  ...
  if (!container || !health || typeof TerminalCtor !== 'function' || typeof FitAddonCtor !== 'function') throw new Error('Linux 4 terminal runtime is unavailable');
  ...
  const vm = new Runtime({
    ...
    screen_container: document.getElementById('linux4-screen'),
  ...
  vm.add_listener('serial0-output-byte', (value?: number) => {
    ...
    term.write(ch);
    if (!ready && /(?:login:|\$\s*$|#\s*$|Buildroot|Linux version)/im.test(serial)) {
      ready = true;
      health.dataset.state = 'ready';
      health.setAttribute('aria-label', 'Linux 4 ready');
      document.getElementById('linux4-status')!.textContent = 'Linux 4 • ready';
    }
  });

  health.dataset.state = 'booting';
  document.getElementById('linux4-status')!.textContent = 'Linux 4 • booting';
}
