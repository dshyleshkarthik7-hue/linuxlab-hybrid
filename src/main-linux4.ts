...
async function boot(): Promise<void> {
  ...
  try {
    ...
    vm = new V86Runtime({ wasm_path: '/v86.wasm', memory_size: enforcer.memoryBytes, vga_memory_size: enforcer.vgaMemoryBytes, screen_container: screen, bios: { url: '/api/v86-firmware/seabios.bin' }, vga_bios: { url: '/api/v86-firmware/vgabios.bin' }, cdrom: { buffer: iso }, boot_order: 0x20, autostart: true, disable_speaker: true, net_device: { type: 'none' } });
