...
type V86 = { serial0_send: (data: string) => void; add_listener: (event: string, callback: (value?: number) => void) => void; keyboard_send_text?: (data: string) => void; };
...
async function boot(): Promise<void> {
  ...
  if (!container || !health || !status || !screen) throw new Error('Linux 4 terminal runtime is unavailable');
  ...
  term.writeln('LinuxTerminal — Ultra Light Linux 4'); status.textContent = 'Linux 4 • loading';
  ...
  let serial = ''; let ready = false;
  const markReady = () => { if (ready) return; ready = true; health.dataset.state = 'ready'; health.setAttribute('aria-label', 'Linux 4 ready'); status.textContent = 'Linux 4 • ready'; };
  health.dataset.state = 'booting'; status.textContent = 'Linux 4 • booting';
  const vm = new Runtime({ wasm_path: '/v86.wasm', memory_size: 256 * 1024 * 1024, vga_memory_size: 8 * 1024 * 1024, screen_container: screen, bios: { url: '/seabios.bin' }, vga_bios: { url: '/vgabios.bin' }, cdrom: { buffer: iso }, boot_order: 0x20, autostart: true, disable_speaker: true, net_device: { type: 'none' } });
  vm.add_listener('emulator-ready', () => markReady());
  vm.add_listener('serial0-output-byte', (value?: number) => { if (typeof value !== 'number') return; const ch = String.fromCharCode(value & 255); serial = (serial + ch).slice(-16384); term.write(ch); if (/(?:login:|\$\s*$|#\s*$|Buildroot|Linux version)/im.test(serial)) markReady(); });
}
window.addEventListener('DOMContentLoaded', () => { void boot().catch(error => { const message = error instanceof Error ? error.message : String(error); const health = document.getElementById('linux4-health'); if (health) { health.dataset.state = 'offline'; health.setAttribute('aria-label', 'Linux 4 offline'); } const status = document.getElementById('linux4-status'); if (status) status.textContent = `Linux 4 • ${message}`; const terminal = document.getElementById('linux4-terminal'); if (terminal) terminal.textContent = `Linux 4 failed to start: ${message}`; }); });
