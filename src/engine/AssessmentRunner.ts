...
export interface TestCase { id:number; description:string; injectedVar:{name:string;value:number}; expectedSubstring:string; }
...
export class AssessmentRunner {
  ...
  private evaluateSuite(code:string,lang:'c'|'java',suite:TestCase[],structural:Array<[string,RegExp]>):AssessmentResult {
    ...
    for(const tc of suite){let output='';try{output=this.engine.executeGeneralCode(code,lang,{[tc.injectedVar.name]:tc.injectedVar.value});}catch(e){output=String(e)}const passed=output.includes(tc.expectedSubstring);checks.push({label:tc.description,passed,feedback:passed?'Expected output produced.':`Expected output containing: ${tc.expectedSubstring}`});}
