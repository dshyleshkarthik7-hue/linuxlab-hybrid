...
export class StructuredLinuxEngine extends InBrowserLinuxEngine {
  ...
  private async invokeLegacyCommand(command: string, stdin: string, signal: AbortSignal): Promise<{ output: string; exitCode: number }> {
    ...
    this.throwIfAborted(signal); const output = await this.executeCommand(command, stdin); this.throwIfAborted(signal); let exitCode = Number.isInteger(this.exitCode) ? this.exitCode : 0; if (exitCode === 0 && this.isFailureDiagnostic(output)) exitCode = 1; return { output, exitCode };
  }
