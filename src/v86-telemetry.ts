...
export interface GuestTelemetryCallbacks {
  onTelemetry?: (snapshot: GuestTelemetrySnapshot) => void;
...
export function attachGuestTelemetry(vm: V86TelemetryTarget, callbacks: GuestTelemetryCallbacks = {}): () => void {
  ...
  const collect = (): void => {
    ...
    requested = true;
    try {
      vm.serial0_send(TELEMETRY_REQUEST);
    } catch (error) {
      requested = false;
      console.debug('[LinuxTerminal] guest telemetry request failed', error);
    }
  };
  ...
  const onSerial = (value?: number): void => {
    ...
    if (!identityOpen || end <= marker) return;
  ...
  const onReady = (): void => {
    if (disposed || requested || requestTimer !== null) return;
