...
export class V86LinuxTerminal {
 ...
 private async start(profile:Profile):Promise<void>{
  ...
  const id=++this.bootId; await this.dispose(); this.profile=profile; this.ready=false; this.serial=''; this.bootPromptSent=false; this.bootCommand='alpine console=tty0 console=ttyS0,115200'; this.showTerminal(); this.term.clear();
