...
export class V86LinuxTerminal {
  ...
  private async start(profile: Profile): Promise<void> {
    const id = ++this.bootId; this.bootController?.abort(); this.bootController = new AbortController(); const signal = this.bootController.signal; await this.dispose();
    this.profile = profile; this.enforcer = new VMRuntimeResourceEnforcer(VM_RESOURCE_POLICIES[profile.policy]); this.ready = false; this.guestIdentity = null; this.setHealth('booting'); this.serial = ''; this.lastSerialAt = 0; this.bootPromptSent = false; this.bootStage = 'starting'; this.showTerminal(); this.term.clear();
    if (!profile.supported) { this.setHealth('offline'); this.status(profile.name + ' • incompatible with browser VM'); this.monitor('Not booted • ' + (profile.arch || 'unknown architecture')); this.term.writeln('LinuxTerminal — ' + profile.name); this.term.writeln('\r\n[Compatibility] ' + (profile.note || 'This image cannot run in this browser emulator.')); return; }
    this.status(profile.name + ' • checking runtime'); this.term.writeln('LinuxTerminal — ' + profile.name);
    try {
      await this.loadRuntime(); if (id !== this.bootId || signal.aborted) return; await this.preflightRuntimeAssets(signal); if (id !== this.bootId || signal.aborted) return;
      this.status(profile.name + ' • verifying image'); const artifact = artifactForIsoUrl(profile.cdrom); const isoBytes = await fetchVerifiedIso(profile.cdrom, signal); if (isoBytes.byteLength === 0) throw new Error('Verified Linux image is empty'); this.status(profile.name + ' • integrity verified');
      const screen = document.getElementById('screen_container'); if (!screen) throw new Error('VM screen container is missing'); const policy = this.enforcer; if (!policy) throw new Error('VM resource policy was not initialized'); this.bootStage = 'creating VM'; this.status(profile.name + ' • booting'); this.monitor(`${profile.memoryMiB} MiB • verified ${artifact.filename}`);
      const Runtime = window.V86; if (typeof Runtime !== 'function') throw new Error('Local v86 runtime did not expose window.V86');
      ...
      this.emulator = vm; this.sessionTimer = window.setTimeout(() => this.enforceSessionExpiry(id), policy.remainingSessionMs());
      this.telemetryDispose = attachGuestTelemetry(vm, { onTelemetry: snapshot => { if (id !== this.bootId) return; const monitor = document.getElementById('v86-monitor'); if (monitor) monitor.textContent = `REAL guest • ${snapshot.kernel} ${snapshot.architecture} • ${snapshot.cpuPercent.toFixed(1)}% CPU • ${Math.round(snapshot.memoryBytes / 1024 / 1024)} MiB used`; }, onIdentity: identity => { if (id !== this.bootId) return; this.acceptGuestIdentity(identity); } });
      vm.add_listener('serial0-output-byte', (value?: number) => { if (id === this.bootId && typeof value === 'number') this.serialOutput(value); }); this.scheduleBootWatchdog(id, policy.policy.bootTimeoutMs); this.fit();
    } catch (error) { if (id === this.bootId) this.error(error instanceof Error ? error.message : String(error)); }
  }

  private acceptGuestIdentity(identity: GuestIdentity): void { this.guestIdentity = identity; if (identity.kind === this.profile.expectedGuest) { this.status(this.profile.name + ' • ' + (identity.kind === 'alpine' ? 'Alpine identity verified' : 'Buildroot identity verified')); this.monitor(this.profile.memoryMiB + ' MiB • guest identity verified'); this.markReadyIfIdentityVerified(); } }
  private markReadyIfIdentityVerified(): void { if (this.ready || !this.guestIdentity || this.guestIdentity.kind !== this.profile.expectedGuest) return; this.ready = true; this.bootStage = 'guest identity verified'; if (this.bootTimeout !== null) clearTimeout(this.bootTimeout); this.bootTimeout = null; this.setHealth('ready'); this.status(this.profile.name + ' • running'); this.monitor(this.profile.memoryMiB + ' MiB • guest identity verified'); }
  private detectGuestIdentity(text: string): GuestIdentity | null { if (/(?:^|\n)ID=alpine(?:\n|$)/im.test(text) || /Alpine Linux/i.test(text)) return { kind: 'alpine', isAlpine: true, release: text.slice(-4096) }; if (/(?:^|\n)ID=buildroot(?:\n|$)/im.test(text) || /NAME=Buildroot/i.test(text)) return { kind: 'buildroot', isAlpine: false, release: text.slice(-4096) }; return null; }
  private canUseRuntime(): boolean { return Boolean(this.emulator && this.enforcer && !this.enforcer.isStopped() && !this.enforcer.sessionExpired()); }
  private enforceSessionExpiry(id: number): void { if (id !== this.bootId) return; this.enforcer?.stop(); this.error('VM session resource limit reached'); }
  private loadRuntime(): Promise<void> { if (typeof window.V86 === 'function') return Promise.resolve(); if (V86LinuxTerminal.runtimePromise) return V86LinuxTerminal.runtimePromise; V86LinuxTerminal.runtimePromise = new Promise<void>((resolve, reject) => { const script = document.querySelector<HTMLScriptElement>('script[data-linuxlab-v86]'); if (!script) { reject(new Error('Missing static /libv86.js runtime tag')); return; } const finish = () => typeof window.V86 === 'function' ? resolve() : reject(new Error('Local libv86.js loaded but window.V86 was not exposed')); script.addEventListener('load', finish, { once: true }); script.addEventListener('error', () => reject(new Error('Failed to load /libv86.js')), { once: true }); }).catch(error => { V86LinuxTerminal.runtimePromise = null; throw error; }); return V86LinuxTerminal.runtimePromise; }
  private async preflightRuntimeAssets(signal: AbortSignal): Promise<void> { const assets = ['/libv86.js', '/v86.wasm', '/seabios.bin', '/vgabios.bin']; await Promise.all(assets.map(async asset => { const response = await fetch(asset, { method: 'HEAD', cache: 'no-store', signal }); if (!response.ok) throw new Error(`Required VM asset failed to load: ${asset} (${response.status})`); const length = Number(response.headers.get('content-length') || 0); if (!Number.isFinite(length) || length <= 0) throw new Error(`Required VM asset has no valid Content-Length: ${asset}`); })); }
  private scheduleBootWatchdog(id: number, timeoutMs: number): void { if (this.bootTimeout !== null) clearTimeout(this.bootTimeout); this.bootTimeout = window.setTimeout(() => { if (this.ready || id !== this.bootId) return; this.error(`Guest did not become ready within ${Math.round(timeoutMs / 1000)} seconds`); }, timeoutMs); }
  private serialOutput(byte: number): void { if (!this.enforcer?.acceptSerialByte(1)) { this.error('VM serial output limit reached'); return; } const ch = String.fromCharCode(byte & 255); this.lastSerialAt = Date.now(); this.serial = (this.serial + ch).slice(-65_536); const identity = this.detectGuestIdentity(this.serial); if (identity && (!this.guestIdentity || this.guestIdentity.kind !== identity.kind)) this.acceptGuestIdentity(identity); const accepted = this.enforcer.acceptOutput(ch); if (accepted.value) this.term.write(accepted.value); if (!this.bootPromptSent && !this.ready && /\bboot:\s*$/i.test(this.serial)) { this.bootPromptSent = true; this.bootStage = 'bootloader prompt'; this.status(this.profile.name + ' • starting default boot'); this.bootPromptTimer = window.setTimeout(() => { this.bootPromptTimer = null; const vm = this.emulator; if (!vm || !this.canUseRuntime()) return; this.bootStage = 'default boot selected'; if (typeof vm.keyboard_send_text === 'function') vm.keyboard_send_text('\n'); else vm.serial0_send('\n'); }, 250); } const promptDetected = /(?:Welcome to Alpine Linux|OpenRC .*starting up Linux|localhost login:|(?:^|\r?\n)[^\r\n]{0,100}[#$>]\s*$)/mi.test(this.serial); if (!this.ready && promptDetected) { if (!this.guestIdentity || this.guestIdentity.kind !== this.profile.expectedGuest) { const actual = this.guestIdentity?.kind || 'unknown'; this.error(`Guest identity mismatch: expected ${this.profile.expectedGuest}, detected ${actual}`); return; } this.ready = true; this.bootStage = /(?:[#$>]\s*$)/m.test(this.serial) ? 'shell ready' : 'guest running'; if (this.bootTimeout !== null) clearTimeout(this.bootTimeout); this.bootTimeout = null; this.setHealth('ready'); this.status(this.profile.name + ' • ' + (this.bootStage === 'shell ready' ? 'ready' : 'running')); this.monitor(this.profile.memoryMiB + ' MiB • ' + (this.bootStage === 'shell ready' ? 'shell ready' : 'guest running')); } }
  ...
  private showScreen(): void { const screen = document.getElementById('screen_container'); const terminal = document.getElementById('v86-terminal-container'); if (screen) screen.hidden = false; if (terminal) terminal.hidden = true; this.fit(); }
  private status(text: string): void { const element = document.getElementById('v86-status'); if (element) element.textContent = text; }
...
window.addEventListener('DOMContentLoaded', () => { try { const vm = new V86LinuxTerminal(); if (import.meta.env.DEV || location.hostname === '127.0.0.1' || location.hostname === 'localhost') window.linuxLabVM = vm; window.addEventListener('pagehide', () => vm.destroy(), { once: true }); } catch (error) { console.error(error); const container = document.getElementById('v86-terminal-container'); if (container) { container.textContent = 'Unable to start the terminal runtime. Please reload the page and check your browser console for details.'; container.style.color = '#e2e8f0'; container.style.padding = '16px'; } } });