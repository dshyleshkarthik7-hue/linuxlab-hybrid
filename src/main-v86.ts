...
window.addEventListener('DOMContentLoaded',()=>{try{const vm=new V86LinuxTerminal();(window as any).linuxLabVM=vm;window.addEventListener('pagehide',()=>vm.destroy(),{once:true});}catch(e){console.error(e);const container=document.getElementById('v86-terminal-container');if(container){container.textContent='Unable to start the terminal runtime. Please reload the page and check your browser console for details.';container.style.color='#e2e8f0';container.style.padding='16px';}}});
