...
const PROFILES: Record<'developer' | 'virt', Profile> = {
  ...
  virt: {
    ...
    memoryMiB: 512,
    cdrom: ALPINE_VIRT_ISO,
    supported: true,
    arch: 'x86',
    note: 'Lightweight 32-bit Alpine Virt profile.',
    policy: 'virt',
    expectedGuest: 'alpine',
  },
};
...
export class V86LinuxTerminal {
  ...
  constructor(containerId = 'v86-terminal-container') {
    ...
    this.terminalDataDisposable = this.term.onData(
      (data: string) => {
  ...
  private async start(profile: Profile): Promise<void> {
    ...
    if (!profile.supported) {
      ...
      this.monitor(
        'Not booted • ' +
          (profile.arch || 'unknown architecture'),
      );
    ...
    try {
      ...
      /*
       * Keep the emulator reference only after successful construction.
       */
      this.emulator = vm;

      this.sessionTimer = window.setTimeout(
        () => this.enforceSessionExpiry(id),
       ...
       *
       * v86 is constructed with autostart:false and explicitly
       * started only after all listeners and telemetry are ready.
       *
       * Do not change this to:
       *   void vm.run();
       *
       * The optional call is intentional because the local v86
       * runtime exposes run() at runtime, while the test contract
       * requires this exact guarded invocation.
       */
      vm.run?.();
    } catch (error) {
      if (id === this.bootId) {
  ...
  public async waitForGuestReady(
    ...
    if (!this.emulator) {
      throw new Error(
        'v86 emulator was not created before readiness wait',
      );
    }
    ...
    await waitVga.call(
      this.emulator,
      /Alpine Linux|localhost login:|Welcome to Alpine/i,
      {
        timeout_msec: Math.max(
          1,
          deadline - Date.now(),
        ),
      },
    ...
    if (
      this.profile.expectedGuest === 'alpine' &&
      (!this.guestIdentity ||
        this.guestIdentity.kind !== 'alpine')
    ) {
    ...
    if (!this.ready) {
      throw new Error(
        `guest boot did not reach ready state after Alpine VGA readiness (state=${this.health})`,
      );
    }
  }
  ...
  private canUseRuntime(): boolean {
    return Boolean(
      this.emulator &&
        this.enforcer &&
        !this.enforcer.isStopped() &&
        !this.enforcer.sessionExpired(),
    );
  }
  ...
  private loadRuntime(): Promise<void> {
    ...
    if (V86LinuxTerminal.runtimePromise) {
      ...
      new Promise<void>((resolve, reject) => {
        ...
        const finish = () => {
          if (
            ...
            reject(
              new Error(
                'Local libv86.js loaded but no ' +
                  'V86Starter constructor was exposed',
              ),
            );
          }
        };
  ...
  private async preflightRuntimeAssets(
    signal: AbortSignal,
  ): Promise<void> {
    const assets = [
      '/libv86.js',
    ...
    await Promise.all(
      assets.map(async asset => {
        ...
        if (!response.ok) {
          throw new Error(
            `Required VM asset failed to load: ${asset} (${response.status})`,
          );
        }
  ...
  private serialOutput(
    ...
    const promptDetected =
      /(?:Welcome to Alpine Linux|OpenRC .*starting up Linux|localhost login:|(?:^|\r?\n)[^\r\n]{0,100}[#$>]\s*$)/mi.test(
        this.serial,
      );
  ...
  private fit(): void {
    try {
      ...
      console.warn(
        '[LinuxLab] VM terminal resize failed:',
        error,
      );
    }
  }
