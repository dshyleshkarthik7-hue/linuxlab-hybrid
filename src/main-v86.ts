...
export class V86LinuxTerminal {
 ...
 private bootId=0; private ready=false; private health:'booting'|'ready'|'offline'='booting'; private serial=''; private bootTimeout:number|null=null; private bootPromptSent=false; private bootPromptTimer:number|null=null; private bootRetryTimer:number|null=null; private bootStage='starting';
 ...
 private async start(profile:Profile):Promise<void>{
  ...
  const id=++this.bootId; await this.dispose(); this.profile=profile; this.ready=false; this.setHealth('booting'); this.serial=''; this.bootPromptSent=false; this.bootStage='starting'; this.showTerminal(); this.term.clear();
  ...
  try{
   ...
   this.bootTimeout=window.setTimeout(()=>{if(!this.ready&&id===this.bootId){this.bootStage='serial shell timeout';this.setHealth('offline');this.status(this.profile.name+' • VGA may still be running');this.monitor('No serial shell detected');this.term.writeln('\r\n[VM] No serial shell was detected. Open Screen to use the guest console.');}},300000);
   this.fit();
