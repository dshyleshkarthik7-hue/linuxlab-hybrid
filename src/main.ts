...
class LinuxLabApp {
  ...
  private async ensureMonaco(): Promise<void> {
    ...
    if (!this.monacoLoad) this.monacoLoad = this.initMonaco().finally(() => { this.monacoLoad = null; });
  ...
  private async executeTerminalCommand(cmd: string): Promise<void> {
    ...
    try { this.commandHistory.push(cmd); if (this.commandHistory.length > 200) this.commandHistory.shift(); const output = await this.engine.execute(cmd); if (output) this.simTerm.writeln(output); void this.persistSession(); }
  ...
  private async persistSession(): Promise<void> { try { await StorageService.saveSession({ id: this.sessionId, mode: 'simulator', title: 'POSIX Simulator Session', startedAt: this.sessionStartedAt, updatedAt: Date.now() }); } catch (error) { console.warn('[LinuxLab] Session save failed:', error); } }
  ...
  private async clearSavedData(): Promise<void> { const confirmed = window.confirm('Clear all saved workspace files, learning progress, sessions, and quiz attempts? This cannot be undone.'); if (!confirmed) return; try { await StorageService.clearLearningData(); this.commandHistory = []; this.historyCursor = -1; this.sessionId = crypto.randomUUID(); this.sessionStartedAt = Date.now(); const feedback = document.getElementById('test-output-list'); if (feedback) { feedback.replaceChildren(); const notice = document.createElement('span'); notice.style.color = '#fbbf24'; notice.textContent = '✓ Saved learning data cleared. Your current in-memory Linux environment is still running.'; feedback.appendChild(notice); } } catch (error) { console.error('[LinuxTerminal] Failed to clear saved data:', error); window.alert('Saved data could not be cleared. Please try again.'); } }
  private bindEvents(): void { const editorContainer = document.getElementById('monaco-container'); editorContainer?.addEventListener('pointerdown', () => { void this.ensureMonaco(); }, { once: true }); document.getElementById('tab-main-c')?.addEventListener('click', () => { void this.ensureMonaco().then(() => this.switchFileTab('main.c')); }); document.getElementById('tab-main-java')?.addEventListener('click', () => { void this.ensureMonaco().then(() => this.switchFileTab('Main.java')); }); document.getElementById('btn-save-fs')?.addEventListener('click', () => { void this.saveCurrentEditorToFS(); }); document.getElementById('btn-run-tests')?.addEventListener('click', () => { void this.runAutomatedGrading(); }); document.getElementById('btn-clear-data')?.addEventListener('click', () => { void this.clearSavedData(); }); document.getElementById('btn-clear-terminal')?.addEventListener('click', () => { this.simTerm.clear(); this.currentInputBuffer = ''; this.simTerm.write(this.engine.getPrompt()); }); document.querySelectorAll<HTMLButtonElement>('.learning-step').forEach(button => { button.addEventListener('click', () => { document.querySelectorAll('.learning-step').forEach(item => item.classList.remove('active')); button.classList.add('active'); const cmd = button.dataset.cmd ?? ''; this.currentInputBuffer = cmd; this.simTerm.write('\r\n' + this.engine.getPrompt() + cmd); }); }); document.getElementById('btn-reset')?.addEventListener('click', () => { this.currentInputBuffer = ''; this.waitingForProgramInput = false; this.pendingProgramCommand = ''; this.engine = new InBrowserLinuxEngine(); this.attachEditorHook(); this.assessment = new AssessmentRunner(this.engine); void this.ensureMonaco().then(() => this.switchFileTab('main.c')); this.simTerm.clear(); this.simTerm.writeln('\x1b[33m[Virtual Environment Reset Complete]\x1b[0m\r\n'); this.simTerm.write(this.engine.getPrompt()); }); window.addEventListener('keydown', e => { const target = e.target as HTMLElement | null; if (target?.matches('input, textarea, select, [contenteditable="true"]') || target?.closest('.monaco-editor')) return; if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') { e.preventDefault(); void this.saveCurrentEditorToFS(); } if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); void this.runAutomatedGrading(); } }); window.addEventListener('resize', () => { this.editor?.layout(); this.fitSimulatorTerminal('window resize'); }); }
}
