...
export function assertTrustedArtifact(artifact: PinnedArtifact): void {
  ...
  if (!Number.isSafeInteger(artifact.size) || artifact.size <= 0) {
    throw new Error(`Artifact ${artifact.filename} has no trusted exact size configured`);
  }
  if (!trustedUrl(artifact.url, TRUSTED_RELEASE_PREFIX)) {
    throw new Error(`Artifact ${artifact.filename} is not from the trusted release host`);
  }
  if (!trustedUrl(artifact.releaseManifestUrl, TRUSTED_MANIFEST_PREFIX)) {
    throw new Error(`Artifact ${artifact.filename} has an untrusted release manifest URL`);
  }
}
...
export async function verifyArtifact(bytes: ArrayBuffer, artifact: PinnedArtifact): Promise<boolean> {
  ...
  if (bytes.byteLength !== artifact.size) {
    throw new Error(`Artifact ${artifact.filename} has unexpected size (${bytes.byteLength}; expected ${artifact.size})`);
  }
...
export async function verifyResponse(response: Response, artifact: PinnedArtifact): Promise<ArrayBuffer> {
  assertTrustedArtifact(artifact);
  if (!response.ok) {
    throw new Error(`Artifact ${artifact.filename} download failed (${response.status})`);
  }
  const length = response.headers.get('content-length');
  if (length !== String(artifact.size)) {
    throw new Error(`Artifact ${artifact.filename} response has unexpected size metadata`);
  }
