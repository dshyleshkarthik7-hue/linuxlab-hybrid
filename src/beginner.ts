...
type IdentityUser = { email?: string; jwt?: () => Promise<string> };
...
declare global { interface Window { netlifyIdentity?: NetlifyIdentity } }
...
async function tutorToken(): Promise<string | null> { const user = window.netlifyIdentity?.currentUser(); if (!user || typeof user.jwt !== 'function') return null; try { return await user.jwt(); } catch { return null; } }
async function askTutor() {
  const question = tutorQuestion?.value.trim() || ''; if (!question || !tutorOutput || !tutorAsk) return;
  const token = await tutorToken(); if (!token) { showLoginPrompt(); return; }
  tutorAsk.disabled = true; if (tutorStatus) tutorStatus.textContent = 'Thinking…'; tutorOutput.textContent = 'The LinuxTerminal Tutor is reading your learning context…';
  ...
  try {
    const response = await fetch('/api/tutor', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` }, credentials: 'same-origin', body: JSON.stringify({ question, context }) });
    const data: unknown = await response.json(); if (response.status === 401) { showLoginPrompt(); return; }
    if (!response.ok || !data || typeof data !== 'object' || !('answer' in data)) throw new Error('Tutor request failed.');
    const answer = String((data as { answer: unknown }).answer || 'No Tutor response was returned.'); tutorOutput.textContent = answer; tutorHistory.push(`Q: ${question}\nA: ${answer}`); if (tutorStatus) tutorStatus.textContent = 'Ready';
  } catch (error) { tutorOutput.textContent = error instanceof Error ? error.message : 'Tutor is temporarily unavailable. Try the built-in command hints below.'; if (tutorStatus) tutorStatus.textContent = 'Fallback available'; }
  finally { tutorAsk.disabled = false; }
}
...
window.addEventListener('load', () => { const identity = window.netlifyIdentity; if (!identity) return; identity.on('logout', () => { if (tutorStatus) tutorStatus.textContent = 'Signed out'; }); identity.on('login', () => { if (tutorStatus) tutorStatus.textContent = 'Signed in'; }); });
