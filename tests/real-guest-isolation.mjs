...
if (!process.env.REAL_GUEST_BASE_URL) {
  ...
  server = spawn(vite, ['preview', '--host', '127.0.0.1', '--port', String(port), '--strictPort'], {
    ...
    env: { ...process.env, REAL_GUEST_ISO_PROXY: '1' },
...
try {
  ...
  // The test-only Vite preview middleware streams the immutable Release asset
  // directly from the same origin. This avoids Playwright buffering the large ISO
  // and avoids cross-origin/COEP blocking while preserving the app's SHA-256 gate.
