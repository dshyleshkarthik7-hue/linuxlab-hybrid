...
log('browser launch start'); const browser = await chromium.launch({ headless: true, timeout: 15000 }); log('browser launch complete'); const context = await browser.newContext(); const page = await context.newPage();
try {
  ...
  log('real guest isolation gate passed: verified Linux 4 guest, runtime network isolation, integrity, readiness, and resource configuration.');
