...
assert.match(tutor, /count == 1/);
assert.match(tutor, /redis\.call\('EXPIRE', KEYS\[1\], ARGV\[1\]\)/);
assert.equal(/\['INCR',\s*redisKey\]\s*,\s*\['EXPIRE'/.test(tutor), false, 'rate limiter must not refresh TTL on every request');
assert.match(tutor, /status: 429|429/);
assert.match(tutor, /retry-after/);
assert.match(tutor, /available/);
assert.match(tutor, /rateLimit\.allowed/);
