...
assert.match(cspReport, /function clientKey\(context:\s*\{\s*ip\?:\s*string\s*\}\)/);
assert.match(cspReport, /export default async \(request:\s*Request,\s*context:\s*\{\s*ip\?:\s*string\s*\}\)/);
