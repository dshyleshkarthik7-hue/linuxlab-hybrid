...
const STAGE_TIMEOUT_MS = Number(process.env.SMOKE_STAGE_TIMEOUT_MS || 12000);
...
async function stage(page, name, action) {
  ...
  try { await Promise.race([action(), new Promise((_, reject) => setTimeout(() => reject(new Error(`${name} exceeded ${STAGE_TIMEOUT_MS}ms`)), STAGE_TIMEOUT_MS))]); console.log(`Smoke stage passed: ${name} (${Date.now()-started}ms)`); }
  catch (error) {
