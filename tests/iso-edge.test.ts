...
globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (url.includes('/releases/tags/')) return new Response(JSON.stringify({ assets: [{ name: 'alpine-virt-3.24.1-x86.iso', size: sourceSize, digest: 'sha256:9895695d27eabc1e2782598ff0190f7966df8317cc2afe2a6d25360e148a4209' }] }), { status: 200 });
  calls.push(new Request(url, init));
  const body = new Uint8Array(1024);
  return new Response(body, { status: 206, headers: { 'content-length': '1024', 'content-range': `bytes 0-1023/${sourceSize}`, 'content-type': 'application/octet-stream' } });
};
try {
