...
set -Eeuo pipefail
WORK="${1:-}"; OUTPUT="${2:-}"
[[ -n "$WORK" && -n "$OUTPUT" ]] || { echo "WORK and OUTPUT directories are required." >&2; exit 2; }
WORK="$(readlink -f "$WORK")"; OUTPUT="$(readlink -f "$OUTPUT")"
[[ "$WORK" != "/" && "$WORK" != "/tmp" ]] || { echo "Unsafe WORK directory." >&2; exit 1; }
ROOTFS="$WORK/rootfs"; ISO_TREE="$WORK/iso-tree"; BASE_ISO="$WORK/base-iso"; MINIROOTFS="$WORK/alpine-minirootfs-3.24.1-x86.tar.gz"; ALPINE_ISO="$WORK/alpine-standard-3.24.1-x86.iso"; INITRAMFS="$WORK/initramfs-linuxlab-gcc"
...
verify_download(){ local file="$1" sum_url="$2" name="$3" sum_file expected actual; sum_file="${file}.sha256"; curl -fL --retry 5 --retry-delay 2 -o "$sum_file" "$sum_url"; expected="$(awk -v n="$name" '$2==n || $2==("*" n) {print $1; exit}' "$sum_file")"; [[ "$expected" =~ ^[0-9a-fA-F]{64}$ ]] || die "Could not obtain SHA-256 for $name."; actual="$(sha256sum "$file" | awk '{print $1}')"; [[ "$actual" == "$expected" ]] || die "SHA-256 verification failed for $name."; rm -f "$sum_file"; log "Verified $name ($actual)"; }
...
make_node /dev/null 666 1 3; make_node /dev/zero 666 1 5; make_node /dev/random 666 1 8; make_node /dev/urandom 666 1 9; make_node /dev/tty 666 5 0; make_node /dev/console 600 5 1
ln -snf /proc/self/fd "$ROOTFS/dev/fd"; ln -snf /proc/self/fd/0 "$ROOTFS/dev/stdin"; ln -snf /proc/self/fd/1 "$ROOTFS/dev/stdout"; ln -snf /proc/self/fd/2 "$ROOTFS/dev/stderr"
...
set -eu
mkdir -p /root/examples /root/bin /etc/local.d /etc/runlevels/boot /etc/runlevels/default /run
if [ ! -e /sbin/init ]; then if [ -e /sbin/openrc-init ]; then ln -s /sbin/openrc-init /sbin/init; else ln -s /bin/busybox /sbin/init; fi; fi
test -e /sbin/init; test -x /usr/bin/gcc
cat >/tmp/hello.c <<'EOF'
#include <stdio.h>
int main(void){ puts("Hello from LinuxLab GCC!"); return 0; }
EOF
gcc /tmp/hello.c -o /tmp/hello; /tmp/hello; rm -f /tmp/hello.c /tmp/hello
...
echo "__LINUXLAB_READY__"
echo
EOF
chmod +x /etc/local.d/linuxlab.start
rc-update add local default 2>/dev/null || true
cat >/usr/sbin/linuxlab-autologin <<'EOF'
#!/bin/sh
exec login -f root
EOF
chmod +x /usr/sbin/linuxlab-autologin
cat >/etc/inittab <<'EOF'
::sysinit:/sbin/openrc sysinit
::sysinit:/sbin/openrc boot
::wait:/sbin/openrc default
tty0::respawn:/sbin/getty -n -l /usr/sbin/linuxlab-autologin 115200 tty0 vt100
ttyS0::respawn:/sbin/getty -n -l /usr/sbin/linuxlab-autologin 115200 ttyS0 vt100
::shutdown:/sbin/openrc shutdown
EOF
# The kernel calls /init in an initramfs. The previous image incorrectly shipped only /sbin/init.
cat >/init <<'EOF'
#!/bin/sh
...
ALPINE_CONFIG
[[ -x "$ROOTFS/usr/bin/gcc" && -x "$ROOTFS/bin/busybox" && -x "$ROOTFS/init" && -e "$ROOTFS/sbin/init" && -e "$ROOTFS/etc/inittab" ]] || die "Custom Alpine rootfs verification failed."
chroot "$ROOTFS" /bin/sh -c 'set -eu; test -x /init; test -e /sbin/init; test -x /usr/bin/gcc; test -x /bin/busybox; grep -q "ttyS0" /etc/inittab; gcc --version | head -n 1'
cleanup; trap - EXIT
...
gzip -9 -c "$INITRAMFS.cpio" > "$INITRAMFS"; rm -f "$INITRAMFS.cpio"
[[ -s "$INITRAMFS" ]] || die "Initramfs creation failed."
cp -f "$INITRAMFS" "$ISO_TREE/boot/initramfs-lts"
ISOLINUX_BIN=""
for candidate in "$ISO_TREE/boot/syslinux/isolinux.bin" "$ISO_TREE/isolinux/isolinux.bin" "$ISO_TREE/boot/isolinux/isolinux.bin"; do if [[ -f "$candidate" ]]; then ISOLINUX_BIN="$candidate"; break; fi; done
[[ -n "$ISOLINUX_BIN" ]] || die "isolinux.bin was not found."
ISOLINUX_REL="${ISOLINUX_BIN#"$ISO_TREE"/}"; BOOT_CAT="$(dirname "$ISOLINUX_REL")/boot.cat"; [[ -e "$ISO_TREE/$BOOT_CAT" ]] || : > "$ISO_TREE/$BOOT_CAT"
...
file "$OUTPUT/alpine.iso"; ls -lh "$OUTPUT/alpine.iso"; log "LinuxLab Alpine GCC ISO build complete: $OUTPUT/alpine.iso"
