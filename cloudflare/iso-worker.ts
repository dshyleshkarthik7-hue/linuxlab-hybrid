...
function chunkFor(url: URL, size: number): { start: number; end: number } {
  ...
  if (!Number.isSafeInteger(start) || !Number.isSafeInteger(requestedEnd) || start < 0 || requestedEnd < start || start >= size) {
    throw new Error('Chunk outside ISO bounds');
  }
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      chunk = chunkFor(url, source.size);
    } catch {
      return new Response('Invalid chunk', { status: 416, headers });
    }

    const upstream = await fetch(new Request(source.url, {
      method: request.method,
      headers: {
        Accept: 'application/octet-stream',
        'User-Agent': 'LinuxTerminal-ISO-Chunk-Worker/17.0',
    ...
    if (upstream.status !== 206) {
      return new Response('ISO origin unavailable (' + upstream.status + ')', { status: 502, headers });
    }
