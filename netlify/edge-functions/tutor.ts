...
interface RateLimitResult { allowed: boolean; available: boolean; retryAfterSeconds: number }
...
const json = (body: Record<string, unknown>, status: number, origin?: string, extra?: Record<string, string>) => {
  const h = headers(origin);
  for (const [key, value] of Object.entries(extra || {})) h.set(key, value);
  return new Response(JSON.stringify(body), { status, headers: h });
};

function fallback(context: string, question: string) {
  const input = `${question}\n${context}`.toLowerCase();
  if (/\bcommand-not-found\b|\bnot found\b/.test(input)) return 'That command was not found. Check spelling with `command -v <command>`, then verify that the lesson expects that tool.';
  if (/\bpermission denied\b/.test(input)) return 'Permission denied means the current user lacks the required permission. Inspect the target with `ls -l` and learn the ownership and mode before changing anything.';
  if (/\bpwd\b/.test(input)) return '`pwd` prints your current working directory. Use it whenever you need to confirm where you are in the filesystem.';
  if (/\bls\b/.test(input)) return '`ls` lists directory entries. Try `ls -la` to include hidden entries and useful metadata.';
  if (/\bcd\b/.test(input)) return '`cd` changes the current working directory. Try `cd /tmp`, then `pwd` to verify the change.';
  if (/\bmkdir\b/.test(input)) return '`mkdir` creates directories. Start with `mkdir practice`, then use `cd practice` and `pwd` to verify it.';
  if (/\bcat\b/.test(input)) return '`cat` writes a file to standard output. It is useful for short files and for learning how stdout flows through the shell.';
  if (/\b(?:hint|next)\b/.test(input)) return 'Start with the command you just ran, inspect its output and exit status, then make one small change. Compare the new result with your prediction.';
...
async function durableLimit(key: string): Promise<RateLimitResult> {
  if (!UPSTASH_URL || !UPSTASH_TOKEN) return { allowed: false, available: false, retryAfterSeconds: WINDOW_SECONDS };
  const redisKey = `linuxterminal:tutor:${key.replace(/[^a-zA-Z0-9:._-]/g, '_')}`;
  ...
  try {
    const response = await fetch(`${UPSTASH_URL}/pipeline`, {
      ...
      body: JSON.stringify([['EVAL', script, '1', redisKey, String(WINDOW_SECONDS)]]),
    });
    if (!response.ok) return { allowed: false, available: false, retryAfterSeconds: WINDOW_SECONDS };
    const data: unknown = await response.json();
    const result = Array.isArray(data) ? (data[0] as { result?: unknown } | undefined)?.result : undefined;
    const values = Array.isArray(result) ? result : [];
    const count = typeof values[0] === 'number' ? values[0] : LIMIT + 1;
    const ttl = typeof values[1] === 'number' && values[1] > 0 ? values[1] : WINDOW_SECONDS;
    return { allowed: count <= LIMIT, available: true, retryAfterSeconds: Math.max(1, Math.ceil(ttl)) };
  } catch {
    return { allowed: false, available: false, retryAfterSeconds: WINDOW_SECONDS };
  }
}
...
export default async (request: Request, context: unknown) => {
  ...
  if (!UPSTASH_URL || !UPSTASH_TOKEN) return json({ error: 'Tutor rate limiting is not configured.' }, 503, origin, { 'retry-after': String(WINDOW_SECONDS) });

  const user = await authenticatedUser(request);
  if (!user) return json({ error: 'Authentication required. Sign in with Netlify Identity before using Tutor.' }, 401, origin, { 'www-authenticate': 'Bearer realm="linuxterminal-tutor"' });
  ...
  if (contentLength && (!/^\d+$/.test(contentLength) || Number(contentLength) > MAX_BODY_BYTES)) return json({ error: 'request body is too large' }, 413, origin);

  const ip = trustedClientKey(context);
  if (!ip) return json({ error: 'Unable to establish a trusted client identity.' }, 503, origin);
  const rateLimit = await durableLimit(`user:${String(user.id)}:ip:${ip}`);
  if (!rateLimit.available) return json({ error: 'Tutor rate limiting is temporarily unavailable. Please try again shortly.' }, 503, origin, { 'retry-after': String(rateLimit.retryAfterSeconds) });
  if (!rateLimit.allowed) return json({ error: 'Tutor rate limit exceeded. Please try again shortly.' }, 429, origin, { 'retry-after': String(rateLimit.retryAfterSeconds) });
  ...
  const prompt = [
    ...
    'Learner context:', contextText || '(no additional context)', '', 'Question:', question,
  ].join('\n');
  ...
  try {
    const response = await fetch('https://router.huggingface.co/v1/chat/completions', {
      ...
      ], max_tokens: MAX_OUTPUT_TOKENS, temperature: 0.2, stream: false }),
    });
