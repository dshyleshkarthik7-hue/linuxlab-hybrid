...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    const upstreamHeaders=new Headers(); if(range) upstreamHeaders.set('Range',range);
    const upstream=await fetch(source,{method:request.method,headers:upstreamHeaders,redirect:'follow',signal:controller.signal});
    if(!upstream.ok && upstream.status!==206) return new Response('ISO unavailable',{status:upstream.status,headers:cors});
