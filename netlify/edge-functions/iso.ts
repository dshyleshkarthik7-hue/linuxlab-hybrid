...
type ImageSource = { primary: string; releaseApi: string; filename: string; sha256: string; maxBytes: number; size: number };

const IMAGES: Record<ImageName, ImageSource> = {
  developer: { primary: 'https://github.com/dshyleshkarthik7-hue/linuxlab-hybrid/releases/download/v1.0.0/alpine.iso', releaseApi: 'https://api.github.com/repos/dshyleshkarthik7-hue/linuxlab-hybrid/releases/tags/v1.0.0', filename: 'alpine.iso', sha256: '9a4683039f356b6bdfa40897f1985b39d5a02e0f46d477e427e8262401301211', maxBytes: 691011584, size: 691011584 },
  virt: { primary: 'https://github.com/dshyleshkarthik7-hue/linuxlab-hybrid/releases/download/V2.00/alpine-virt-3.24.1-x86.iso', releaseApi: 'https://api.github.com/repos/dshyleshkarthik7-hue/linuxlab-hybrid/releases/tags/V2.00', filename: 'alpine-virt-3.24.1-x86.iso', sha256: '9895695d27eabc1e2782598ff0190f7966df8317cc2afe2a6d25360e148a4209', maxBytes: 51380224, size: 51380224 },
  linux4: { primary: 'https://github.com/dshyleshkarthik7-hue/linuxlab-hybrid/releases/download/v3.00/linux4.iso', releaseApi: 'https://api.github.com/repos/dshyleshkarthik7-hue/linuxlab-hybrid/releases/tags/v3.00', filename: 'linux4.iso', sha256: 'a8ea434ab3b177c55f01275dcc1d35f52cfbee9bd44a32e74765c975b58bcc73', maxBytes: 7731200, size: 7731200 },
};
...
async function verifyReleaseAsset(source: ImageSource): Promise<void> {
  const response = await fetch(source.releaseApi, { headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'LinuxTerminal-ISO-Proxy/11.0' }, cache: 'no-store' });
  if (!response.ok) throw new Error(`Release manifest unavailable (${response.status})`);
  const release = await response.json() as { assets?: Array<{ name?: string; size?: number; digest?: string | null }> };
  ...
  if (!asset || asset.size !== source.size || digest !== source.sha256.toLowerCase()) throw new Error(`Release manifest digest mismatch for ${source.filename}`);
}
...
export default async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    const upstream = await fetch(source.primary, { method: request.method, headers: new Headers({ Accept: 'application/octet-stream', 'User-Agent': 'LinuxTerminal-ISO-Proxy/11.0', 'Accept-Encoding': 'identity' }), redirect: 'follow', signal: controller.signal, cache: 'no-store' });
    if (!upstream.ok) return new Response(`ISO upstream unavailable (${upstream.status})`, { status: 502, headers });
    const contentLengthHeader = upstream.headers.get('content-length');
    if (!contentLengthHeader || Number(contentLengthHeader) !== source.size || Number(contentLengthHeader) > source.maxBytes) return new Response('ISO upstream has an unexpected size', { status: 502, headers });
