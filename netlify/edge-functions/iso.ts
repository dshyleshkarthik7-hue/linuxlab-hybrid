...
function rangeFor(request: Request, size: number): { start: number; end: number } | null {
  ...
  if (!Number.isSafeInteger(start) || !Number.isSafeInteger(end) || start < 0 || end < start || start >= size) throw new Error('Range outside ISO bounds');
...
export default async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!upstream.ok || (range && upstream.status !== 206)) return new Response(`ISO upstream unavailable (${upstream.status})`, { status: 502, headers });

    const contentLengthHeader = upstream.headers.get('content-length');
    const expectedLength = range ? range.end - range.start + 1 : source.size;
    if (contentLengthHeader !== null && (Number(contentLengthHeader) !== expectedLength || Number(contentLengthHeader) > source.size)) {
      return new Response('ISO upstream has an unexpected size', { status: 502, headers });
    }
    if (range) {
      const contentRange = upstream.headers.get('content-range');
      if (contentRange !== `bytes ${range.start}-${range.end}/${source.size}`) return new Response('ISO upstream returned an unexpected range', { status: 502, headers });
    ...
    return new Response('ISO proxy temporarily unavailable', {
      status: 504,
      headers: new Headers({ ...Object.fromEntries(headers), 'Cache-Control': 'no-store' }),
    });
  } finally {
    clearTimeout(timer);
  }
