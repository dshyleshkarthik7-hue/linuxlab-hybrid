...
const V86_INIT_TIMEOUT_MS=15000;
let emulator=null;
let starting=false;
...
async function start(){if(starting)return;starting=true;await stopEmulator();setStatus('Starting…');if(diagnostics){diagnostics.hidden=true;diagnostics.textContent=''}try{if(!screen)throw new Error('VM screen container is missing');const Runtime=globalThis.V86Starter||globalThis.V86;if(typeof Runtime!=='function')throw new Error('Local v86 runtime did not expose a constructor');setStatus('Creating Alpine VM…');emulator=new Runtime({wasm_path:'/v86.wasm',memory_size:RAM_BYTES,vga_memory_size:8*1024*1024,screen_container:screen,bios:{url:`${FIRMWARE_BASE}/seabios.bin`},vga_bios:{url:`${FIRMWARE_BASE}/vgabios.bin`},cdrom:{url:ISO_URL,async:true,size:ISO_SIZE},boot_order:0x213,fastboot:true,bootmenu:false,autostart:false,disable_speaker:true,net_device:{type:'none'}});await waitForV86Loaded(emulator,V86_INIT_TIMEOUT_MS);setStatus('Booting Alpine Linux…');if(typeof emulator.run!=='function')throw new Error('Local v86 runtime does not expose run()');await emulator.run();await waitForAlpine(emulator,BOOT_TIMEOUT_MS);setStatus('Ready — start practicing');screen.focus()}catch(error){console.error(error);fail(error)}finally{starting=false}}
