...
function fail(error){console.error(error);setStatus('Offline — check Reboot');if(diagnostics){diagnostics.hidden=false;diagnostics.textContent=`The Developer Alpine workspace could not start. ${error?.message||'Check your connection and try Reboot.'}`}}
function sleep(ms){return new Promise(resolve=>setTimeout(resolve,ms))}
function waitForV86Loaded(vm,timeoutMs){return new Promise((resolve,reject)=>{let settled=false;let timer;const cleanup=()=>{if(timer)clearTimeout(timer);vm.remove_listener?.('emulator-loaded',onLoaded)};const finish=error=>{if(settled)return;settled=true;cleanup();error?reject(error):resolve()};const onLoaded=()=>finish();try{if(vm?.cpu&&vm?.screen_adapter){finish();return}vm.add_listener('emulator-loaded',onLoaded);timer=setTimeout(()=>finish(new Error('v86 emulator initialization timed out')),timeoutMs)}catch(error){finish(error)}})}
